...
export function EmbeddedWalletSignMethods({
  publicAddress,
}: {
  ...
  const handlePersonalSign = async () => {
    try {
      const signature = await MagicService.magic.wallet.signMessage(PERSONAL_SIGN_PAYLOAD);
      return signature;
    } catch (error) {
      console.error(error);
      throw error;
  ...
  const handleSignTypedDataV1 = async () => {
    try {
      const signature = await MagicService.magic.wallet.signTypedDataV1(SIGN_TYPED_DATA_V1_PAYLOAD);
      return signature;
    } catch (error) {
      console.error(error);
      throw error;
  ...
  const handleSignTypedDataV3 = async () => {
    try {
      const signature = await MagicService.magic.wallet.signTypedDataV3(SIGN_TYPED_DATA_V3_PAYLOAD);
      return signature;
    } catch (error) {
      console.error(error);
      throw error;
  ...
  const handleSignTypedDataV4 = async () => {
    try {
      const signature = await MagicService.magic.wallet.signTypedDataV4(SIGN_TYPED_DATA_V4_PAYLOAD);
      return signature;
    } catch (error) {
      console.error(error);
      throw error;
  ...
  const handleSignTransaction = async () => {
    try {
      const signature = await MagicService.magic.wallet.signTransaction(signTransactionPayload);
      return signature;
    } catch (error) {
      console.error(error);
      throw error;
    ...
    <Tabs defaultValue="personal" className={TabsClasses.root}>
      <div className="p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">Embedded Wallet Signing Methods</h2>
            <p className="text-muted-foreground">
              Test various cryptographic signing operations with Magic SDK
            </p>
          </div>
        </div>
